...
class App extends Component {
  ...
  componentWillReceiveProps({isScriptLoadSucceed}){
    ...
    if (isScriptLoadSucceed) {
      ...
      const map = new window.google.maps.Map(document.getElementById('map'), {
        ...
        center: new window.google.maps.LatLng(55.753960, 37.620393)
      });
  ...
  componentDidMount(){
    ...
    this.state.locations.map((location,index)=>{
      return fetchJsonp(`https://ru.wikipedia.org/w/api.php?action=opensearch&search=${location.title}&format=json&callback=wikiCallback`)
 ...
 render() {
  const {locations, query, requestWasSuccessful} = this.state;
    ...
    return (
      ...
      requestWasSuccessful ? (
        <div>
        <nav className="nav">
        <span id="subject" tabIndex='0'>Moscow sights</span>
        </nav>
        <div id="container">
        <div id="map-container" role="application" tabIndex="-1">
        <div id="map" role="region" aria-label="Moscow Neighborhood"></div>
        </div>
  ...
  export default scriptLoader(
    [`https://maps.googleapis.com/maps/api/js?key=AIzaSyA6vKfyZAgoXDOJdQRnDymYFTzCeXTuXg8&v=3.exp&libraries=geometry,drawing,places`]
    )(App);
